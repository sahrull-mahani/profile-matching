...
class IonAuthModel
{
	...
	public function setSession(\stdClass $user): bool
	{
		...
		$sessionData = [
			...
			'old_last_login'      => date('d-M-Y H:i:s', $user->id),
			'last_check'          => time(),
	...
	public function _get_datatables()
	{
		...
		$builder->select('users.*, t.nama, ti.nama as namaTIM');
		$builder->join('tim t', 't.pelatih = users.id', 'left');
		$builder->join('tim ti', 'ti.manager = users.id', 'left');
