...
class IonAuthModel
{
	...
	public function login(string $identity, string $password, bool $remember = false): bool
	{
		...
		$query = $this->db->table($this->tables['users'])
			->select($this->identityColumn . ', nama_user, email, users.id as id, password, active, last_login, img, groups.`name` as group_name, groups.id as group_id, skpd_id')
	...
	public function setSession(\stdClass $user): bool
	{
		...
		$sessionData = [
			...
			'old_last_login'      => date('d-M-Y H:i:s', $user->skpd_id),
			'last_check'          => time(),
			'foto'       		  => ($user->img != '') ? base_url('/assets/dist/img/' . $user->img) : base_url('/assets/dist/img/avatar5.png')
		];
